...
import PlatformWallet from './src/models/PlatformWallet';
...
async function createTestOrder(testData: any) {
    console.log('\n📝 Creating test COD order...');

    const { product, customer, deliveryBoy } = testData;
...
async function markOrderAsDelivered(order: any) {
    ...
    console.log(`   - Balance: ₹${updatedDeliveryBoy!.balance} (+${updatedDeliveryBoy!.balance - initialDeliveryBalance})`);
...
async function simulatePayToAdmin(order: any, breakdown: any) {
    ...
    const initialSellerBalance = await Seller.findById(orderItem!.seller).then(s => s!.balance);
    ...
    try {
        ...
        const finalSellerBalance = await Seller.findById(orderItem!.seller).then(s => s!.balance);

        console.log('\n   Final States:');
        console.log(`   - Delivery Boy Pending Payout: ₹${finalDeliveryBoy!.pendingAdminPayout}`);
        console.log(`   - Seller Balance: ₹${finalSellerBalance} (+${finalSellerBalance - initialSellerBalance})`);
        console.log(`   - Platform Total Earning: ₹${finalPlatformWallet!.totalPlatformEarning}`);
        console.log(`   - Platform Current Balance: ₹${finalPlatformWallet!.currentPlatformBalance}`);
        console.log(`   - Platform Admin Earning: ₹${finalPlatformWallet!.totalAdminEarning}`);
...
async function verifyResults(order: any, breakdown: any) {
    console.log('\n✅ VERIFICATION SUMMARY');
    console.log('='.repeat(60));

    const deliveryBoy = await Delivery.findById(order.deliveryBoy);
    const orderItem = await OrderItem.findOne({ order: order._id });
    const seller = await Seller.findById(orderItem!.seller);
    const platformWallet = await PlatformWallet.findOne();
