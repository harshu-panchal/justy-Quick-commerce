...
import { getExecutives, updateExecutive, deleteExecutive } from '../../services/adminExecutiveService';
...
export default function AdminExecutives() {
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm('Are you sure you want to permanently delete this executive account? This action cannot be undone.')) {
            return;
        }

        try {
            await deleteExecutive(id);
            toast.success('Executive account deleted successfully');
            fetchExecutives();
        } catch (error: any) {
