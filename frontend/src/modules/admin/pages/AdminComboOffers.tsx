...
export default function AdminComboOffers() {
    ...
    const getProductPrice = (id: string) => {
        ...
        if (p.variations && p.variations.length > 0) {
            return p.variations[0].discPrice || p.variations[0].price || 0;
        }
        return 0;
    };

    const handleToggleComboProduct = (id: string) => {
        setSelectedComboProducts((prev) =>
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            updated = combos.map((c) => (c.id === editingId ? newCombo : c));
        } else {
    ...
    const calculateOriginalTotal = () => {
        ...
        const comboPrices = selectedComboProducts.reduce((s, id) => s + getProductPrice(id), 0);
                                        ...
                                        >
                                            ...
                                            <td className="p-4 align-middle">
                                                <div className="flex flex-wrap gap-1">
                                                    {combo.comboProductNames.map((name, idx) => (
                                                        <span
                                                            key={idx}
